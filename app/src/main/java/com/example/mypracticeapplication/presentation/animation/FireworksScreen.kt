...
fun FireworksScreen(
    ...
    val trailPath = remember { Path() } // Cache Path to avoid allocations per frame
                ...
                Canvas(modifier = Modifier.fillMaxSize()) {
                    ...
                    if (progress < 0.7f) {
                        ...
                        for (i in burstParticles.indices) {
                            ...
                            // Optimization: Direct multiplication instead of pow(2)
                            val gravity = (burstProgress * burstProgress) * 200
                            ...
                            // Optimization: use native alpha parameter
                            drawCircle(
                    ...
                    for (i in fireworks.indices) {
                        ...
                        if (fireworkProgress < firework.explosionDelay) {
                            ...
                            // Optimization: Reuse Path object and rewind
                            trailPath.rewind()
                            trailPath.moveTo(firework.startX, currentY)
                            for (j in 1..firework.trailLength) {
                                val currentTrailY = currentY + j * 8f
                                val wobble = sin(j * 0.5f + fireworkProgress * 20) * 3
                                trailPath.lineTo(firework.startX + wobble, currentTrailY)
                            }
                        ...
                        // Explosion phase
                            ...
                            // Optimization: Use pre-generated sparks
...
private fun generateFireworks(count: Int): List<Firework> {
    return List(count) { index ->
        ...
        // Pre-generate sparks for this firework
