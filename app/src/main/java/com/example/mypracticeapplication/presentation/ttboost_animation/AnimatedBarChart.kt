 ...
 * @param entranceProgress Animation progress for bar entrance (0.0 to 1.0) - passed as lambda to defer read
...
fun AnimatedBarChart(
    ...
    // BOLT OPTIMIZATION: Cache the brush to avoid allocation during draw phase
    val barBrush = remember {
        Brush.verticalGradient(colors = TtBoostTheme.BarGradient)
    }
    ...
    Canvas(modifier = modifier.fillMaxSize()) {
        ...
        // BOLT OPTIMIZATION: Use indexed for loop instead of forEachIndexed to avoid iterator allocation per frame
