...
fun AnimatedBarChart(
    ...
    // Optimization: Cache CornerRadius and pre-calculate pixel values outside of the draw loop
    val cornerRadius = remember { CornerRadius(35f, 35f) }
    val density = LocalDensity.current
    val barWidthPx = remember(barWidth, density) { with(density) { barWidth.toPx() } }
    val spacingPx = remember(barSpacing, density) { with(density) { barSpacing.toPx() } }

    Canvas(modifier = modifier.fillMaxSize()) {
        val currentEntranceProgress = entranceProgress()

        // Optimization: Use indexed for loop instead of forEachIndexed to avoid iterator allocation per frame
