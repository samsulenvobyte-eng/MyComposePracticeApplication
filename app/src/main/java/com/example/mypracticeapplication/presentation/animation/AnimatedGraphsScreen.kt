...
fun AnimatedDonutChart(
    ...
    // Performance optimization: Cache the Stroke object to avoid per-frame allocations
    ...
    Box(
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            ...
            // Performance optimization: Cache animation value locally to minimize state reads in the loop
            val progress = animationProgress.value

            // Performance optimization: Replace forEachIndexed with standard loop to avoid iterator allocation
...
fun AnimatedLineChart(
    ...
    // Performance optimization: Use primitive arrays for coordinates to avoid per-frame List<Offset> allocations
    ...
    Canvas(modifier = modifier) {
        ...
        // Performance optimization: Cache animation values locally
        val progress = animationProgress.value
        val vBounce = verticalBounce.value
        ...
        for (i in dataPoints.indices) {
            ...
            val bouncedY = baselineY + (distanceFromBaseline * vBounce)
        ...
        if (dataPoints.size >= 2) {
            // Performance optimization: Reuse path instances by calling rewind()
            linePath.rewind()
            ...
            // Performance optimization: Reuse path instances
            fillPath.rewind()
                ...
                if (showDataPoints) {
                    for (i in dataPoints.indices) {
                        ...
                        if (progress >= pointProgress) {
                            ...
                            // Performance optimization: Use alpha parameter to avoid Color object creation
                            drawCircle(
                                color = lineColor,
                                alpha = pointAlpha,
