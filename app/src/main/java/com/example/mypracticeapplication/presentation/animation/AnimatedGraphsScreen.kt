...
fun AnimatedDonutChart(
    ...
    // Optimization: Cache Stroke object to avoid per-frame allocations
    val donutStroke = remember(strokeWidth, density) {
    ...
    Box(
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            ...
            val radius = (canvasSize.minDimension - donutStroke.width) / 2
            val center = Offset(canvasSize.width / 2, canvasSize.height / 2)
            
            var currentStartAngle = startAngle
            
            // Optimization: Use indexed for loop to avoid iterator allocation
            for (i in 0 until segments.size) {
                ...
                if (showLabels && animationProgress.value > 0.5f) {
                    ...
                    val labelRadius = radius + donutStroke.width / 2
...
fun AnimatedLineChart(
    ...
    // Optimization: Cache Path objects to avoid per-frame heap allocations
    val linePath = remember { Path() }
    val fillPath = remember { Path() }

    // Optimization: Cache coordinate arrays to avoid boxing from List<Offset>
    ...
    val density = LocalDensity.current
    // Optimization: Cache Stroke and Brush objects
    ...
    Canvas(modifier = modifier) {
        ...
        // Optimization: Use indexed for loop and primitive arrays to avoid iterator and Offset allocations
        ...
        if (dataPoints.size >= 2) {
            // Optimization: Reuse and rewind Path objects instead of creating new ones every frame
            linePath.rewind()
