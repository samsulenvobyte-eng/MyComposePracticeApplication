...
fun AnimatedDonutChart(
    ...
    val arcStroke = remember(strokeWidth, density) {
    ...
    Box(
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            ...
            for (i in segments.indices) {
                ...
                val sweepAngle = sweepAngles[i] * progress
...
fun AnimatedLineChart(
    ...
    // Pre-allocate paths and coordinate arrays to avoid per-frame allocations
    ...
    Canvas(modifier = modifier) {
        ...
        // Calculate all points with vertical bounce applied - use for loop to avoid iterator allocation
        ...
        for (i in dataPoints.indices) {
            ...
            pointsX[i] = x
            pointsY[i] = baselineY + (distanceFromBaseline * bounceValue)
        }
        ...
        if (dataPoints.size >= 2) {
            ...
            clipRect(
                ...
                if (showDataPoints) {
                    val progress = animationProgress.value
                    val pointRadius = dataPointRadius.toPx()
                    for (i in dataPoints.indices) {
