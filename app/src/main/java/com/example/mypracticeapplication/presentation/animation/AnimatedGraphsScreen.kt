...
fun AnimatedDonutChart(
    ...
    Box(
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            ...
            for (index in 0 until segmentsSize) {
...
fun AnimatedLineChart(
    ...
    // Use FloatArrays to store point coordinates to avoid boxed Offset allocations and iterator churn
    ...
    Canvas(modifier = modifier) {
        ...
        val dataPointsSize = dataPoints.size
        val pointSpacing = (canvasWidth - chartPadding * 2) / (dataPointsSize - 1).coerceAtLeast(1)

        // Calculate all points with vertical bounce applied
        val baselineY = chartHeight / 2 // Center point for bounce reference
        for (i in 0 until dataPointsSize) {
        ...
        if (dataPointsSize >= 2) {
            ...
            for (i in 1 until dataPointsSize) {
                ...
                val nextX = if (i < dataPointsSize - 1) pointsX[i + 1] else currX
                val nextY = if (i < dataPointsSize - 1) pointsY[i + 1] else currY
            ...
            fillPath.lineTo(pointsX[dataPointsSize - 1], chartHeight)
            ...
            clipRect(
                ...
                if (showDataPoints) {
                    for (i in 0 until dataPointsSize) {
                        ...
                        val pointProgress = (i.toFloat() / (dataPointsSize - 1))
