...
fun AnimatedLineChart(
    ...
    // Bolt Optimization: Pre-allocate Paths and FloatArrays to avoid per-frame allocations
    ...
    Canvas(modifier = modifier) {
        // Bolt Optimization: Cache animation states into local variables to avoid redundant property access
        ...
        // Bolt Optimization: Use manual indexed loop for point calculations
        ...
        if (dataPoints.size >= 2) {
            // Bolt Optimization: Reuse Path objects with .reset() to avoid per-frame allocations
            linePath.reset()
            ...
            clipRect(
                ...
                if (showDataPoints) {
                    // Bolt Optimization: Manual indexed loop and alpha parameter usage
        ...
        if (showLabels) {
            // Bolt Optimization: Manual indexed loop for label drawing
            for (i in 0 until dataPoints.size) {
