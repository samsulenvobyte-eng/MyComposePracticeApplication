...
fun AnimatedLineChart(
    ...
    // Performance Optimization: Cache paths and arrays to avoid per-frame allocations
    ...
    Canvas(modifier = modifier) {
        ...
        // Performance Optimization: Cache high-frequency state values
        ...
        // Performance Optimization: Use manual for loop to avoid iterator allocation
        ...
        if (size >= 2) {
            // Performance Optimization: Reuse path objects with rewind()
            linePath.rewind()
            ...
            for (i in 1 until size) {
                ...
                val nextX = if (i < size - 1) pointsX[i + 1] else currX
                val nextY = if (i < size - 1) pointsY[i + 1] else currY
            ...
            // Performance Optimization: Reuse fill path
            fillPath.rewind()
            fillPath.addPath(linePath)
            fillPath.lineTo(pointsX[size - 1], chartHeight)
            ...
            clipRect(
                ...
                if (showDataPoints) {
                    for (i in 0 until size) {
                        ...
                        val pointProgress = (i.toFloat() / (size - 1))
                        if (progress >= pointProgress) {
                            // Fade in the point
                            val pointAlpha = ((progress - pointProgress) * 5f).coerceIn(0f, 1f)
                            // Performance Optimization: Use alpha parameter to avoid Color.copy()
                            drawCircle(
                                color = lineColor,
                                alpha = pointAlpha,
